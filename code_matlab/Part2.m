...
figure
scatter(x_t, y_t_plus_1, 10, customBlue);
...
scatter(x_t, y_t_plus_1, 10, customBlue);
hold on;
scatter(x_t, M_theoretical, 8, 'filled');
for j = 1:length(bandwidths)
    scatter(x_t, M_estimated_gaussian(:, j), 4, 'filled');
end
title('Estimated M(x) with Gaussian Kernel');
xlabel('x_t');
ylabel('M(x)');
legend(['Theoretical M(x)', arrayfun(@(h) ['h=', num2str(h)], bandwidths, 'UniformOutput', false)]);

% 3. Plot estimated conditional mean with Epanechnikov kernel
figure
hold on;
scatter(x_t, M_theoretical);
for j = 1:length(bandwidths)
    scatter(x_t, M_estimated_epanechnikov(:, j));
end
